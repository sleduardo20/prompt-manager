...
export const SidebarContent = ({ prompts }: SidebarContentProps) => {
  ...
  const expandSidebar = () => setIsCollapset(false);
  const handleNewPrompt = () => router.push("/new");
  return (
    <aside
      className={`border-r border-gray-700 flex flex-col h-full bg-gray-800 
    transition-[transform,width] duration-300 ease-in-out fixed  left-0 top-0 z-50 md:z-auto w-[80vw] sm:w-[320px] ${isCollapsed ? "md:w-[72px]" : "md:w-[384px]"}`}
    >
      {isCollapsed && (
